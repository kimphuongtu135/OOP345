...
void loadData(const char* filename, sdds::Autoshop& as)
{
	...
	while (file)
	{
		...
		try {
			...
			std::cout << "Unrecognized record type: [" << ch << "]" << endl;
		}
...
int main(int argc, char** argv)
{
	...
	std::list<const sdds::Vehicle*> vehicles;
	{
		...
		auto fastVehicles = [](const sdds::Vehicle * vehi)
		{
			if (vehi->topSpeed() > 300) {
				return true;
			}
			else
				return false;
	...
	std::cout << std::endl;
	{
		...
		auto brokenVehicles = [](const sdds::Vehicle * vehi)
		{
			if (vehi->condition() == "broken") {
				return true;
			}
			else
				return false;
